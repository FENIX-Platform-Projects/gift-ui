define([
    ...
    var s = {
        ...
        WIDTH: 500,
        level_number: 1,
        total: "#pieThreeLevDrilldown-total",
        process: {
            first_level_process: [
                {
    ...
    function ThreeLevDrilldown(params) {
        ...
        this._getProcessedResourceForChart(s.process.total_process)
            .then(
                _.bind(this._onTotalSuccess, this),
    ...
    ThreeLevDrilldown.prototype._onTotalSuccess = function (success) {

        var r = success.data.length > 0 ? success.data[0] : [],
            total = Formatter.format(r[0]) + " " + r[2];
        console.log(total)
        if (r[0]){
            $(s.total).html(total);
        } else {
    ...
    ThreeLevDrilldown.prototype._processTotalValueSeries = function (resource) {
